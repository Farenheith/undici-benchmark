...
function createPureUndiciH2Client() {
  if (!undiciAgentH2) {
    undiciAgentH2 = new undici.Agent({
      allowH2: true,
      connections: 1,
...
async function runBenchmark() {
  ...
  └─ Requests/sec (median): ${(TOTAL_REQUESTS / (axiosHttp1Data.median / 1000)).toFixed(2)}`;
    ...
    const pureUndiciH2Summary = `undici HTTP/2 Results (median of ${SAMPLING} runs):
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ...
  └─ Requests/sec (median): ${(TOTAL_REQUESTS / (pureUndiciH1Data.median / 1000)).toFixed(2)}`;
    ...
    console.log('\n--- undici vs axios (HTTP/2) ---');
    const undiciVsAxios =
      ((axiosHttp2Data.median - pureUndiciH2Data.median) /
        axiosHttp2Data.median) *
      100;
    console.log(
      `  ${undiciVsAxios > 0 ? '✓' : '✗'} undici is ${Math.abs(undiciVsAxios).toFixed(2)}% ${undiciVsAxios > 0 ? 'faster' : 'slower'}`,
    );

    console.log('\n--- Ranking (HTTP/2 only, median times) ---');
    const http2Results = [
      { name: 'axios', time: axiosHttp2Data.median, stdDev: axiosHttp2Data.stdDev },
      { name: 'undici', time: pureUndiciH2Data.median, stdDev: pureUndiciH2Data.stdDev },
    ...
    const http1Results = [
      ...
      { name: 'undici', time: pureUndiciH1Data.median, stdDev: pureUndiciH1Data.stdDev },
    ].sort((a, b) => a.time - b.time);
    ...
    const clients = [
      ...
      { name: 'undici', h2: pureUndiciH2Data.median, h1: pureUndiciH1Data.median },
    ];
