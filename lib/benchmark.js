...
const undiciAgentH1 = new undici.Agent({
  allowH2: false,
  connections: 1,
  maxCachedSessions: 10,
  pipelining: 1024,
...
async function runConcurrentBenchmark(client, concurrency, totalRequests, url) {
  ...
  for (let i = 0; i < totalRequests; i += concurrency) {
    ...
    // Log any rejections and count them — failure counts matter: a run with
    // ECONNREFUSED bursts (e.g. HTTP/1.1 ephemeral-port pressure) has a
    // median computed over fewer successful requests, which skews the ranking.
  ...
  return { results, totalTime, failures };
}
...
async function runBenchmarkMultipleTimes(client, concurrency, totalRequests, url, runs = SAMPLING) {
  ...
  for (let i = 0; i < runs; i++) {
    const { totalTime, failures: runFailures } = await runConcurrentBenchmark(client, concurrency, totalRequests, url);
  ...
  return { times, median, avg, stdDev, failures };
}
...
async function runBenchmark() {
  ...
  ├─ Failures: ${axiosHttp2Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((axiosHttp2Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${axiosHttp1Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((axiosHttp1Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${pureUndiciH2Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((pureUndiciH2Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${minimalUndiciH2Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((minimalUndiciH2Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${requestUndiciH2Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((requestUndiciH2Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${abortUndiciH2Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((abortUndiciH2Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  ├─ Total requests per run: ${TOTAL_REQUESTS}
  ├─ Failures: ${pureUndiciH1Data.failures} / ${SAMPLING * TOTAL_REQUESTS} (${((pureUndiciH1Data.failures / (SAMPLING * TOTAL_REQUESTS)) * 100).toFixed(2)}%)
  ...
  └─ Requests/sec (median): ${(TOTAL_REQUESTS / (pureUndiciH1Data.median / 1000)).toFixed(2)}`;
    ...
    // Warn when a leg dropped requests — median over fewer successes is skewed
    const h1Failures = axiosHttp1Data.failures + pureUndiciH1Data.failures;
    const h2Failures =
      axiosHttp2Data.failures +
      pureUndiciH2Data.failures +
      minimalUndiciH2Data.failures +
      requestUndiciH2Data.failures +
      abortUndiciH2Data.failures;
    if (h1Failures > 0) {
      console.log(`\n⚠️  HTTP/1.1 leg dropped ${h1Failures} requests (ECONNREFUSED bursts on Windows are expected) — treat its medians as noisy.`);
    }
    if (h2Failures > 0) {
      console.log(`\n⚠️  HTTP/2 leg dropped ${h2Failures} requests — investigate before trusting the h2 ranking.`);
    }
